...
def coordinates_for_grid(start_coord):
    ...
    repeat = [start_coord[0] + 1.5, start_coord[1] - 1]
    ...
    coord_array = [bottom_right, bottom_left, top_left, top_right, repeat]
    return coord_array


def grid_coordinates():
    # start coordinate for each row
    start_coord_and_row_iterations = [[[141.75, -10], 1], [[129, -11], 6], [[141, -11], 2], [[129, -12], 6], [[141, -12], 2], [[126, -13], 1], [[129, -13], 6], [[141, -13], 2], [[123, -14], 10], [[141, -14], 3], [[123, -15], 10], [[141, -15], 3], [[121.5, -16], 17], [[121.5, -17], 17], [[121.5, -18], 17], [[118.5, -19], 20], [[114, -20], 24], [[114, -21], 25], [[112.5, -22], 26], [[112.5, -23], 27], [[112.5, -24], 28], [[112.5, -25], 28], [[112.5, -26], 27], [[114, -27], 27], [[114, -28], 27], [[114, -29], 27], [[114, -30], 27], [[115.5, -31], 25], [[115.5, -32], 9], [[132, -32], 14], [[114, -33], 7], [[133.5, -33], 12], [[114, -34], 4], [[121.5, -34], 2], [[135, -34], 11], [[115.5, -35], 2], [[136.5, -35], 10], [[139.5, -36], 7], [[139.5, -37], 7], [[141, -38], 5], [[144.2, -39.62], 3], [[144.2, -40.62], 3], [[144.2, -41.62], 3], [[144.2, -42.62], 3]]
...
def grid_adjuster(output_list):
    output_len = len(output_list)
    for i in range(output_len):
        bottom_right_coord = output_list[i][2][0]
        bottom_left_coord = output_list[i][2][1]
        top_left_coord = output_list[i][2][2]
        top_right_coord = output_list[i][2][3]
        repeat_coord = output_list[i][2][4]
        bottom_right_coord[0] -= 0.05
        bottom_right_coord[1] += 0.05
        bottom_left_coord[0] += 0.05
        bottom_left_coord[1] += 0.05
        top_left_coord[0] += 0.05
        top_left_coord[1] -= 0.05
        top_right_coord[0] -= 0.05
        top_right_coord[1] -= 0.05
        repeat_coord[0] -= 0.05
        repeat_coord[1] += 0.05
        new_grid = [bottom_right_coord, bottom_left_coord, top_left_coord, top_right_coord, repeat_coord]
        output_list[i][2] = new_grid
    return output_list


if __name__ == '__main__':
    grids = grid_coordinates()
    adjusted_grids = grid_adjuster(grids)
    print(adjusted_grids)
